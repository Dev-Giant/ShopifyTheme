      ...
      $.ajax({
        ...
        success: function(item) {
          ...
          for (i=0; i < itemDiscounts.length; i++) {
            ...
            discountMessage = '<span class="notification-discount"><b>'+Shopify.translation.cart_discount+':</b> ' + title + ' (- <span class="money">' + amount + '</span>)</span>';
          }
          ...
          window.setTimeout(function(){
            ...
            var currentCheckout = Shopify.theme_settings.shop_url +'/checkout'; //blank to avoid breaking checkout link

            if(Shopify.theme_settings.current_locale !== defaultLanguage) {
              var currentCheckout = Shopify.theme_settings.shop_url +'/'+ Shopify.theme_settings.current_locale + '/checkout';
            }
...
selectCallback = function(variant, selector) {
  ...
  if (variant && variant.available == true) {
    if(variant.price < variant.compare_at_price){
      ...
      $('.was-price, .selected-price', $product).addClass('sale');
    } else {
      $('.was-price', $product).html('');
      $('.current-price', $product).parent().removeClass('sale');
      $('.was-price, .selected-price', $product).removeClass('sale');
    }
      
    $('.selected-price', $product).html('<span class="money">' + Shopify.formatMoney(variant.price, $('body').data('money-format')) + '</span>');
    ...
    if (variant.price > 0) {
      ...
      $('.current-price', $product).html("Free");
    }
